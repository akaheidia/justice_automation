*** Variables ***

${JUS_URL}                   https://10.54.143.205
${JUS_HOST_IP}               10.54.143.205
${JUS_USERNAME}              root
${JUS_PASSWORD}              n7830466

${XMC_URL}                   https://10.54.143.201:8443
${XMC_HOST_IP}               10.54.143.201
${XMC_HOSTNAME}              justice-XMC-201.extremenetworks.com
${XMC_USERNAME}              root
${XMC_PASSWORD}              n7830466
${XMC_SERVER_LOG}            /usr/local/Extreme_Networks/NetSight/appdata/logs/server.log
${XMC_NSJBOSS_PROPERTIES}    /usr/local/Extreme_Networks/NetSight/appdata/NSJBoss.properties

${ANALYTICS_HOST_IP}         10.54.143.202
${ANALYTICS_HOSTNAME}        Justice-Analytics-202
${ANALYTICS_USERNAME}        root
${ANALYTICS_PASSWORD}        n7830466

${XMC_2_URL}                 https://10.54.143.203:8443
${XMC_2_HOST_IP}             10.54.143.203
${XMC_2_HOSTNAME}            Justice-XMC-203
${XMC_2_USERNAME}            root
${XMC_2_PASSWORD}            n7830466
${XMC_2_SERVER_LOG}          /usr/local/Extreme_Networks/NetSight/appdata/logs/server.log
${XMC_2_NSJBOSS_PROPERTIES}  /usr/local/Extreme_Networks/NetSight/appdata/NSJBoss.properties

${LIN_DOWNLOADS_DIR}         /home/hwhite/Downloads
${WIN_DOWNLOADS_DIR}         C:\\Users\\root\\Downloads
${DOWNLOADS_DIR}             ${LIN_DOWNLOADS_DIR}
